...
To get the SPECFEM3D Globe software package, type this:
{\small
\begin{verbatim}
...
\noindent
SPECFEM3D Globe now supports CUDA, OpenCL and HIP GPU acceleration.
...
SPECFEM3D Globe also supports CUDA-aware MPI. This code feature can be enabled by adding the flag \texttt{-{}-enable-cuda-aware-mpi} to
...
\item[\texttt{GPU\_RUNTIME}] sets the runtime environments: $1$ for CUDA, $2$ for OpenCL, $3$ for HIP
 and $0$ for compile-time decision (hence, SPECFEM3D Globe should
...
(e.g., \texttt{-{}-prefix}) do not apply to SPECFEM3D Globe.
\end{description}
