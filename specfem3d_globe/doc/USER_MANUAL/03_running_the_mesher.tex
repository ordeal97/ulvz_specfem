...
The spectral-element mesh for a SPECFEM3D Globe simulation is based
...
%
\item [{\texttt{1D\_Berkeley}}] A 1D transversely isotropic Earth model developed by \citet{FrRo2014} as reference model for SEMUCB-WM1.
%
\end{description}
%%
for Mars:
...
%
\item [{\texttt{mars\_1D}}] 1D Mars model as defined in file \texttt{mars\_1D.dat} in folder \texttt{DATA/mars}.
%
\end{description}
%%
for Moon:
...
%
\item [{\texttt{EMC\_model}}] Reads in an IRIS EMC model provided by the user. For more details how to compile and setup the code, please see the README info in folder \texttt{DATA/IRIS\_EMC/}.
%
\item [{\texttt{SEMUCB\_A3d}}] A global radially anisotropic shear velocity model from spectral-element waveform tomography developed by \citet{FrRo2014} at Berkeley.
%
\item [{\texttt{SEMUCB\_A3d\_3dQ}}] The elastic global Berkeley model (SEMUCB) combined with the 3D attenuation model developed by \citet{KaRo2018}.
%
\end{description}
%%
for Mars:
...
The crustal model is defined by the files in folder \texttt{DATA/crustmaps/marscrust*.cmap}
%
\item [{\texttt{mars\_SH\_model}}] For a user-specified 3D Mars model given in spherical harmonics and provided in folder \texttt{DATA/mars/SH\_model/}. The model files need to be specified in file \texttt{SH\_model\_files.dat}. The spherical harmonic file format follows the Mars evolution model outputs developed by \citet{Plesa2021}. SH models define both crustal and mantle depths, with an isotropic seismic model description (Vp, Vs, and density).
%
\end{description}
...
Thus, flattening f = 1/299.8 is what is used in SPECFEM3D Globe, as it should. And eccentricity squared $e^2 = 1 - (1-f)^2 = 1 - (1 - 1/299.8)^2 = 0.00665998813529$, and the correction factor used in the code to convert geographic latitudes to geocentric is $1 - e^2 = (1-f)^2 = (1 - 1/299.8)^2 = 0.9933400118647$.
%
...
to save \href{http://www.avs.com}{AVS}, \href{http://www.opendx.org}{OpenDX}, or \href{http://www.paraview.org}{ParaView}
...
and \texttt{OUTPUT\_SEISMOS\_SAC\_BINARY}.
One can choose any combination of these
parameters (details on the formats follow in the description of each
parameter). \href{http://www.iris.edu/software/sac/}{SAC} is a signal-processing software
...
station name, etc., see Appendix~\ref{cha:SAC-headers}). For details on the format, please check the \href{http://www.iris.edu/software/sac/}{SAC webpage}. Files will be named with extension \texttt{.sacan}.
%
...
For details on the binary format please also check the \href{http://www.iris.edu/software/sac/}{SAC webpage} and Appendix~\ref{cha:SAC-headers}. Files will be named with extension \texttt{.sac}.
%
...
that one should not use SPECFEM3D Globe with the regular mesh and
...
Regarding fluid/solid coupling at the CMB and ICB, in SPECFEM3D Globe
...
The SPECFEM3D Globe memory requirements can be estimated before or
...
%-----------------------------------------------------------------------------------------------------------------------------------%
\section{Scattering perturbations}\label{sec:Scattering-perturbations}
%-----------------------------------------------------------------------------------------------------------------------------------%

SPECFEM3D Globe also allows to add optional scattering perturbations to the elastic model parameters.
