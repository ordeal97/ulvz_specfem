...
from ulvz_model_postprocess.input_contracts import parse_labeled_path, validate_databases_mpi
...
def run(args: argparse.Namespace) -> dict:
    ...
    extractor = _require_extractor(args.extractor)
    out_dir = Path(args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    inspect_dir = out_dir / "extractor_inspect"
    summary = {
        "status": "PASS",
        "model_label": label,
    ...
    if args.extract_mode == "summary":
        _run_inspect(extractor, path, inspect_dir)
        summary["extractor_inspect_manifest"] = str(inspect_dir / "extractor_layout_manifest.json")
...
def _require_extractor(value: str | None) -> Path:
    if not value:
        raise ModelPostprocessError(
            "raw SPECFEM DATABASES_MPI validation/extraction requires explicit "
            "--extractor /path/to/specfem3d_globe/bin/xulvz_model_extract"
        )
    extractor = Path(value)
    if not extractor.exists():
        raise ModelPostprocessError(
            f"raw SPECFEM DATABASES_MPI validation/extraction requires --extractor; "
            f"missing extractor: {extractor}"
        )
    if not extractor.is_file():
        raise ModelPostprocessError(f"--extractor must be a file: {extractor}")
    return extractor


def _run_inspect(extractor: Path, database_dir: Path, out_dir: Path) -> None:
    command = [str(extractor), "--inspect", str(database_dir), str(out_dir)]
    result = subprocess.run(command, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False)
    if result.returncode != 0:
        details = "\n".join(part for part in [result.stdout.strip(), result.stderr.strip()] if part)
        raise ModelPostprocessError(f"DATABASES_MPI inspection failed through xulvz_model_extract: {details}")
    manifest_path = out_dir / "extractor_layout_manifest.json"
    if not manifest_path.exists():
        raise ModelPostprocessError("xulvz_model_extract --inspect completed but did not write extractor_layout_manifest.json")
