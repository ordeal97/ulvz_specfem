...
| Multi-ULVZ source interface | Array-backed runtime `ulvz_body_t` implementation accepts legacy single-body files and `N_ULVZ=0..N`; PREM and S40RTS preserve their established formulas and apply at most one body per point | Lightweight two-rank tests cover legacy/new one-body equivalence, explicit zero baseline, distinct three-body parameters, permutation invariance, broadcast reinitialization, and parser/overlap failures. A real NCHUNKS=1, NEX=32, two-rank S40RTS+CRUST2.0 mesher pair also passes: matched `N_ULVZ=0`, then valid `N_ULVZ=3`, normalized provenance, complete databases, and direct per-body physical-field audit. | Evidence: `results/multi_ulvz_real_mesher_validation_20260911T131745Z/`. The preceding retained run correctly rejected an overlapping fixture, providing real guard evidence. Overlap is intentionally forbidden using the current CMB circular-cap support. No solver, bsub, production mesh/database, or waveform simulation was run. |
...
## 2026-07-20 two-chunk planner Event 1 performance acceptance

真实 Event 1 的 partial phase-aware 输入请求 127 个台站的 S/Sdiff：返回 127 条 S 路径、
127 个缺失 Sdiff pair 和 66,650 个路径点。NEX=448 只影响资源建议，不改变 orientation
候选数量或路径点数。路径边界核心已从逐点 Python scalar 改为固定全球路径向量和批量旋转
边界的 NumPy 计算；真实核心基准由 2.487 s/orientation 降至约 0.0028 s/orientation
（8-orientation block）。

第三轮持久验收已完成两次完整真实 CLI：12:31.62（RSS 264,464 KiB）和 12:28.80
（RSS 267,248 KiB），均退出码 0；两套 planner_output 递归严格一致。最终 package
pytest 为 20 passed。相对上一轮约 18.5 分钟，墙钟时间降低约 32%。旧版真实运行仅有
1800 s 超时证据，因此只能给出旧新版加速下界，不能给出精确旧新版速度比。

端到端正确性与可重复性验收已完成；10 分钟性能目标未达到。此前诊断记录了 719.94 s 的
batch path evaluation、60 次 keeper scalar review 共 352.17 s，以及约 203 s 未分类时间；
第三轮正式时间未打开详细 profiling，故不猜测剩余时间归因。权威证据为
