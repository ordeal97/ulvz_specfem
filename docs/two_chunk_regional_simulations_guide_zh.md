...
`globe.png` 显示其球面闭合关系；`geometry_audit.json` 与 `candidates.json` 给出 source/station 的 chunk label、local
...
3. 打开 `PLAN_DIR/map.png` 与 `PLAN_DIR/globe.png` 查看真实地理和球面闭合的 AB/AC
   boundaries。继续前先读取 chosen candidate
